...
export default function Intake() {
  ...
  const [matcher, setMatcher] = useState<string>("ai"); const [lang, setLang] = useState<"en"|"ms">("en");
  ...
  async function parse(){ setErr(""); try{ const data = await postJSON<Parsed>(API("/parse"), { text: raw, matcher, lang }); setRes(data);} catch(e:any){ setErr(e?.message ?? "Failed to parse"); } }
  async function createOrder(){ const code = (res?.match as any)?.order_code || window.prompt("Enter new order code"); if(!code) return; try{ await postJSON(API("/orders"), { code }); alert("Order created: "+code);}catch(e:any){ alert(e?.message ?? "Failed to create order"); } }
  ...
  return (<div className="grid cols-2">
    <section className="card stack"><div className="row">
      ...
      <select className="input" style={{maxWidth:140}} value={matcher} onChange={e=>setMatcher(e.target.value)}>
        <option value="ai">AI</option><option value="hybrid">Hybrid</option><option value="rules">Rules</option></select>
      ...
      <div className="row"><button className="btn" onClick={createOrder}>Create Order</button></div>
    </section>
    <section className="card stack"><div className="title">{t("Result","Keputusan")}</div>
      ...
      {res && (<>
        <div className="row"><span className="badge">{t("Parsed","Diparse")}</span>{res.match?.order_code && <span className="badge">#{res.match.order_code} <span className="small">({res.match.reason})</span></span>}</div>
